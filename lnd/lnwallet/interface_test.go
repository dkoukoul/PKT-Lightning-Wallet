...
func assertTxInWallet(t *testing.T, w *lnwallet.LightningWallet,
	...
	// status.
	txs, err := w.ListTransactionDetails(0, btcwallet.UnconfirmedHeight, 0, 0, 0, false, 0,0)
	if err != nil {
...
func testListTransactionDetails(miner *rpctest.Harness,
	...
	txDetails, errr := alice.ListTransactionDetails(
		startHeight, chainTip, 0, 0, 0, false, 0,0,
	)
	if errr != nil {
	...
	// mined yet.
	txDetails, errr = alice.ListTransactionDetails(
		chainTip, btcwallet.UnconfirmedHeight, 0, 0, 0, false,0,0,
	)
	if errr != nil {
		t.Fatalf("unable to fetch tx details: %v", errr)
	}
	...
	txDetails, errr = alice.ListTransactionDetails(
		chainTip, chainTip, 0, 0, 0, false, 0,0,
	)
	if errr != nil {
		t.Fatalf("unable to fetch tx details: %v", errr)
	}
